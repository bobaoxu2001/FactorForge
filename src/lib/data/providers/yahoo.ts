...
export async function fetchYahooHistoricalPrices(
  ...
  try {
    ...
    if (!response.ok) {
      return buildFallbackPrices(symbol, range, `Yahoo Finance request failed: HTTP ${response.status}`);
    }
    ...
    if (!result || !quote || timestamps.length === 0) {
      return buildFallbackPrices(symbol, range, payload.chart?.error?.description ?? "Yahoo Finance returned empty data");
    }
    ...
    if (prices.length < 40) {
      return buildFallbackPrices(symbol, range, "Yahoo Finance returned insufficient daily bars; fallback is active");
    }
    ...
    return buildFallbackPrices(symbol, range, `Yahoo Finance network failure: ${message}`);
  }
}
